...
export const AdminVerificationsPage: React.FC = () => {
  const { verificationRequests, adminApproveVerification, adminRejectVerification } = useData();

  const [searchTerm, setSearchTerm] = useState('');
  const [filterStatus, setFilterStatus] = useState<string>('pending');
  const [rejectingVer, setRejectingVer] = useState<VerificationRequest | null>(null);
  const [rejectReason, setRejectReason] = useState('');
  const [copiedId, setCopiedId] = useState<string | null>(null);

  const filteredVerifications = verificationRequests.filter((v) => {
    const matchesFilter = filterStatus === 'all' || v.status === filterStatus;
    const matchesSearch =
      v.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
      v.userName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      v.userEmail.toLowerCase().includes(searchTerm.toLowerCase()) ||
      v.trxId.toLowerCase().includes(searchTerm.toLowerCase()) ||
      v.senderNumber.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleConfirmReject = (e: React.FormEvent) => {
    e.preventDefault();
    if (!rejectingVer || !rejectReason.trim()) return;
    adminRejectVerification(rejectingVer.id, rejectReason.trim());
    setRejectingVer(null);
    setRejectReason('');
  };
    ...
    <div className="space-y-8">
      ...
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">
          ৳15 User Account Verification Queue
        </h1>
        <p className="text-xs sm:text-sm text-slate-500">
          Verify user identity and ৳15 verification fee TrxID. Approving activates the Blue Verified Trust Badge.
        </p>
      </div>
              ...
              {filteredVerifications.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-12 text-center text-slate-400">
                    No verification requests match this criteria.
                  </td>
                </tr>
              ) : (
                filteredVerifications.map((v) => (
                  <tr key={v.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="p-4">
                      <p className="font-mono font-bold text-slate-900 dark:text-white">#{v.id}</p>
                      <p className="text-[11px] text-slate-400">{v.submittedAt}</p>
                    </td>
                    <td className="p-4">
                      <p className="font-semibold text-slate-900 dark:text-white">{v.userName}</p>
                      <p className="text-[11px] text-slate-400">{v.userEmail}</p>
                    </td>
                    <td className="p-4">
                      <span className="font-bold text-slate-800 dark:text-slate-200 uppercase">{v.paymentMethod}</span>
                      <p className="text-[11px] font-mono text-slate-500">{v.senderNumber}</p>
                    </td>
                    <td className="p-4">
                      <div className="flex items-center gap-1.5">
                        <span className="font-mono font-bold text-blue-600 dark:text-blue-400">{v.trxId}</span>
                        <button
                          onClick={() => handleCopyTrx(v.trxId, v.id)}
                    ...
                    <td className="p-4 text-right">
                      {v.status === 'pending' ? (
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => adminApproveVerification(v.id)}
                            className="px-3 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs shadow-xs transition-colors flex items-center gap-1"
                          >
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <h3 className="font-bold text-base text-red-600 dark:text-red-400">
              Reject Verification #{rejectingVer.id}
            </h3>
            ...
            <form onSubmit={handleConfirmReject} className="space-y-4 text-xs">
              <div>
                ...
                >
                  ...
                  <option value="Incorrect payment amount (must be ৳15)">Incorrect payment amount (must be ৳15)</option>
